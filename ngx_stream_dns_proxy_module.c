...
static void ngx_stream_dns_proxy_finalize(ngx_stream_session_t *s,
                                          ngx_uint_t rc);
...
/******************** dns write filter ***************************/
static ngx_int_t ngx_stream_dns_write_filter(ngx_stream_session_t *s,
                                             ngx_chain_t *in,
                                             ngx_uint_t from_upstream);

static ngx_int_t ngx_stream_dns_write_filter_init(ngx_conf_t *cf);

static ngx_int_t ngx_stream_variable_dns_answer_context(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_question_context(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_record_context(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_dns_add_vars(ngx_conf_t *cf);

static ngx_int_t ngx_stream_variable_dns_question_id(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_question_flags(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_answer_reply_code(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_answer_flags(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_question_num(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_answer_num(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_authority_num(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_int_t ngx_stream_variable_dns_additional_num(
    ngx_stream_session_t *s, ngx_stream_variable_value_t *v, uintptr_t data);

static ngx_stream_variable_t ngx_stream_dns_variables[] = {

    {ngx_string("dns_answer_content"), NULL,
     ngx_stream_variable_dns_answer_context, 0, 0, 0},

    {ngx_string("dns_query_content"), NULL,
     ngx_stream_variable_dns_question_context, 0, 0, 0},

    {ngx_string("dns_query_record"), NULL,
     ngx_stream_variable_dns_record_context, 0, 0, 0},

    {ngx_string("dns_query_id"), NULL, ngx_stream_variable_dns_question_id, 0,
     0, 0},

    {ngx_string("dns_query_flags"), NULL,
     ngx_stream_variable_dns_question_flags, 0, 0, 0},

    {ngx_string("dns_reply_code"), NULL,
     ngx_stream_variable_dns_answer_reply_code, 0, 0, 0},

    {ngx_string("dns_reply_flags"), NULL, ngx_stream_variable_dns_answer_flags,
     0, 0, 0},

    {ngx_string("dns_question_num"), NULL, ngx_stream_variable_dns_question_num,
     0, 0, 0},

    {ngx_string("dns_answer_num"), NULL, ngx_stream_variable_dns_answer_num, 0,
     0, 0},

    {ngx_string("dns_authority_num"), NULL,
     ngx_stream_variable_dns_authority_num, 0, 0, 0},

    {ngx_string("dns_additional_num"), NULL,
     ngx_stream_variable_dns_additional_num, 0, 0, 0},

    {ngx_null_string, NULL, NULL, 0, 0, 0}};

static ngx_command_t ngx_stream_dns_proxy_commands[] = {

    {ngx_string("dns_proxy_pass"), NGX_STREAM_SRV_CONF | NGX_CONF_TAKE12,
     ngx_stream_dns_proxy_pass, NGX_STREAM_SRV_CONF_OFFSET, 0, NULL},

    {ngx_string("dns_proxy_connect_timeout"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_dns_proxy_srv_conf_t, connect_timeout), NULL},

    {ngx_string("dns_proxy_timeout"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
     ngx_conf_set_msec_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_dns_proxy_srv_conf_t, timeout), NULL},

    {ngx_string("dns_decode_packet_enable"),
     NGX_STREAM_MAIN_CONF | NGX_STREAM_SRV_CONF | NGX_CONF_FLAG,
     ngx_conf_set_flag_slot, NGX_STREAM_SRV_CONF_OFFSET,
     offsetof(ngx_stream_dns_proxy_srv_conf_t, decode_packet_enable), NULL},
...
static int ngx_stream_dns_check_dns_label_valid(char *str) {
  char *res = strchr(str, '.');
  int labelLen;
  // Check if last label
  if (res == NULL) { // Last label
    ...
    if (memcmp(str, internatDNS, strlen(internatDNS)) !=
        0) { // Dosnt contain international encoding
  ...
  for (char *pos = str; pos != str + (labelLen * sizeof(char));
    ...
    if ((!isalpha(*pos)) && (!isdigit(*pos)) && (*pos != '-') &&
        (*pos != '_')) {
      return 0; // False
    }
  }
...
invalid/false */
static int ngx_stream_dns_check_dns_valid(char *str) {
  // NULL check
  if (str == NULL) {
    return 0; // False
  }
  ...
  char *dot = str;
  if (ngx_stream_dns_check_dns_label_valid(dot) == 0) {
    return 0; // False
  }
  dot = strchr(str, '.');
  while (dot != NULL) {
...
static void ngx_stream_dns_proxy_handler(ngx_stream_session_t *s) {
  ...
  if (ctx == NULL) {
    ctx = ngx_pcalloc(c->pool, sizeof(ngx_stream_dns_proxy_ctx_t));
    if (ctx == NULL) {
      return;
    }
    ...
    ctx->question_msg.question =
        ngx_list_create(c->pool, 1, sizeof(ngx_dns_question_t));
    ctx->question_msg.answer =
        ngx_list_create(c->pool, 3, sizeof(ngx_dns_rr_t));
    ctx->answer_msg.question =
        ngx_list_create(c->pool, 1, sizeof(ngx_dns_question_t));
    ctx->answer_msg.answer = ngx_list_create(c->pool, 3, sizeof(ngx_dns_rr_t));
...
static void ngx_stream_dns_proxy_init_upstream(ngx_stream_session_t *s) {
  ...
  if (c->buffer && c->buffer->pos < c->buffer->last) {
    ...
    if (pscf->type != NGX_CONF_UNSET && c->type != pscf->type) {
      proxy_flag = 1;
    } else {
      proxy_flag = 0;
    }
...
static void ngx_stream_dns_proxy_process(ngx_stream_session_t *s,
  ...
  /*ngx_log_debug3(NGX_LOG_DEBUG_STREAM, c->log, 0,
                 "ngx_stream_dns_proxy_process() form_upstream: %d, c sock type:
     %s, p socke type: %s", from_upstream, c->type == SOCK_STREAM
     ?"SOCK_STREAM":"SOCK_DGRAM", pc->type == SOCK_STREAM
...
static char *ngx_stream_dns_proxy_merge_srv_conf(ngx_conf_t *cf, void *parent,
  ...
  ngx_conf_merge_msec_value(conf->connect_timeout, prev->connect_timeout, 6000);

  ngx_conf_merge_msec_value(conf->timeout, prev->timeout, 6000);
  ...
  ngx_conf_merge_value(conf->decode_packet_enable, prev->decode_packet_enable,
                       1);
...
static ngx_int_t ngx_stream_variable_dns_answer_context(
  ...
  for (i = 0;; i++) {
    ...
    if (answer[i].rtype == TypeA || answer[i].rtype == TypeAAAA ||
        answer[i].rtype == TypePTR || answer[i].rtype == TypeCNAME ||
        answer[i].rtype == TypeTXT || answer[i].rtype == TypeNS) {
      // name
      p = ngx_cpymem(p, answer[i].name.data, answer[i].name.len);
      // ttl class type
      p = ngx_snprintf(p, context_len, " %d %s %s ", answer[i].rttl,
      ...
      p = ngx_snprintf(p, context_len, " %d %d %d %d %d", soa->serial,
                       soa->refresh, soa->retry, soa->expire, soa->minttl);
    } else if (answer[i].rtype == TypeMX) {
      p = ngx_cpymem(p, answer[i].name.data, answer[i].name.len);
      mx = (ngx_dns_mx_t *)(answer[i].rdata);
      // ttl class type
      p = ngx_snprintf(p, context_len, " %d %s %s %d ", answer[i].rttl,
                       ngx_dns_class_type_string(answer[i].rclass),
                       ngx_dns_type_string(answer[i].rtype), mx->preference);
...
static ngx_int_t ngx_stream_variable_dns_record_context(
  ...
  if (ngx_stream_dns_check_dns_valid(question->name.data)) {
    goto notfind;
  }
